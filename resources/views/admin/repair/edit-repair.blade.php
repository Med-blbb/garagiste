...
<div class="container col-md-6 mx-auto mt-5">
    <form action="{{ route('admin.update-repair', ['id' => $repair->id]) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input type="date" class="form-control" name="start_date" id="start_date" value="{{ $repair->start_date }}">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="client_notes" id="client_notes" value="{{ $repair->client_notes }}">
        </div>
        <div class="form-group">
            <label for="mechanic_id">Mechanic ID</label>
            <input type="text" class="form-control" name="mechanic_id" id="mechanic_id" value="{{ $repair->mechanic_id }}">
        </div>
        <div class="form-group">
            <label for="vehicle_id">Vehicle ID</label>
            <input type="text" class="form-control" name="vehicle_id" id="vehicle_id" value="{{ $repair->vehicle_id }}">
        </div>
        <button type="submit" class="btn btn-primary">Submit</button>
        <a href="{{ route('admin.show-repair') }}" class="btn btn-secondary" style="margin-left: 10px">Back</a>
    </form>
</div>
@endsection